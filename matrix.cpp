...
double matrix::averaging(int i,int j){
    if(dim%2==1){
        ...
        else return (4*value[i][j] +2*(value[i+1][j]+value[i-1][j]+value[i][j+1]+value[i][j-1]) +1*(value[i+1][j+1]+value[i-1][j+1]+value[i+1][j-1]+value[i-1][j-1]))/16.0;
    }
    else{
        if(i==0 && j==0) return (4*value[i][j] +2*(value[i+1][j]+value[i][j+1]) +1*value[i+1][j+1])/9.0;
...
double matrix::insertion(int i,int j,int dim_in){
    ...
    else if(i%2==1 && j%2==1) return (value[(i+1)/2][(j+1)/2] + value[(i+1)/2][(j-1)/2]+value[(i-1)/2][(j+1)/2] + value[(i-1)/2][(j-1)/2])/4.0;
}
...
matrix matrix::Residual(const matrix & rho){
    ...
    for(int i=0;i<dim;i++){
        for(int j=0;j<dim;j++){
            if(i!=0 && i!=dim-1 &&j!=0 && j!=dim-1){
                res.value[i][j] = 0.25*(this->value[i+1][j]+this->value[i-1][j]+this->value[i][j+1]+this->value[i][j-1] -this->value[i][j]*4- h*h*rho.value[i][j])/h/h;
            }
        }
    }
    return res;
